...
class APCAnovaCommand(str, Enum):
...
@dataclass
class APCWifiJobStatus:
    cook_time_remaining: int | None
    state: APCAnovaState


@dataclass
class APCWifiPinInfo:
...
@dataclass
class APCWifiSystemInfoNxp:
    firmware_version: str  # 'version-string'


@dataclass
class APCWifiTemperatureInfo:
...
@dataclass
class APCWifiCookerStateBody:
    audio_control: Any | None
    boot_id: str | None
    cap_touch: Any | None
    heater_control: Any | None
    job: APCWifiJob
    job_status: APCWifiJobStatus
    motor_control: Any | None
    network_info: Any | None
    pin_info: APCWifiPinInfo
    system_info: APCWifiSystemInfo | None
    system_info_3220: APCWifiSystemInfo3220 | None
    system_info_nxp: APCWifiSystemInfoNxp | None
    temperature_info: APCWifiTemperatureInfo
    ...
    def to_apc_update(self) -> APCUpdate:
        ...
        binary_sensors = APCUpdateBinary(
            cooking=bool(self.job.mode == APCAnovaMode.COOK),
            preheating=bool(self.job_status.state == APCAnovaState.PREHEATING),
            maintaining=bool(
                self.job_status.state == APCAnovaState.MAINTAINING
                or self.job_status.state == APCAnovaState.TIMER_EXPIRED
            ),
...
def build_wifi_cooker_state_body(apc_response: dict[str, Any]) -> APCWifiCookerStateBody:
    system_info = None
    ...
    job = APCWifiJob(
        ...
        mode=APCAnovaMode(job_json["mode"]),
    ...
    job_status = APCWifiJobStatus(
        ...
        state=APCAnovaState(job_status_json["state"]),
    )
    ...
    if system_info_3220_json:
        system_info_3220 = APCWifiSystemInfo3220(
    ...
    if system_info_nxp_json:
        system_info_nxp = APCWifiSystemInfoNxp(
